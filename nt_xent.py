...
class NTXentLoss(torch.nn.Module):
    ...
    def _get_similarity_function(self, use_cosine_similarity):
        if use_cosine_similarity:
            ...
            # TODO check MICCAI 2021 which loss to use
    ...
    def forward(self, zis, zjs): # i and j are augmented pairs
...
if __name__ == '__main__':
    loss_function = NTXentLoss(device=0, temperature=0.1, use_cosine_similarity=False)
    x = torch.tensor(np.random.rand(4, 128)).to(0)
    y = torch.tensor(np.random.rand(4,128)).to(0)
    # import pdb; pdb.set_trace()
